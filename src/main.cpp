...
void setup()
{
    ...
    const uint64_t t_start_us = esp_timer_get_time();
    ...
    const uint16_t batt_mv = utils_battery_voltage_mv();
    const int16_t tc10 = utils_internal_temp_c10();
    const uint64_t uptime_now_us = rtc_total_us + (uint64_t)esp_timer_get_time();
    const uint32_t uptime_now_s = (uint32_t)(uptime_now_us / 1000000ULL);

    (void)lora_send_packet(irr, batt_mv, tc10, uptime_now_s);

    const uint64_t active_us = esp_timer_get_time() - t_start_us;
    rtc_total_us += active_us;
    const uint64_t sleep_us = (active_us >= TARGET_PERIOD_US) ? 1000ULL : (TARGET_PERIOD_US - active_us);
